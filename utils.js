...
const DOM = {
    ...
    argonMemory:
        document.getElementById("argonMemory")
...
function getSelectedArgon2Memory() {
    ...
    return Number.parseInt(DOM.argonMemory?.value || State.argonMemoryKib, 10);

}

function updateArgonMemoryUI() {

    if (!DOM.argonMemory) return;

    DOM.argonMemory.value = String(State.argonMemoryKib);
    DOM.argonMemory.disabled = State.busy || State.mode === "decrypt";
